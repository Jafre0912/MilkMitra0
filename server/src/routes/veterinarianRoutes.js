      ...
      try {
        ...
        try {
          ...
          return res.json({
            success: true,
            message:
              "Gemini API is working properly with fallback model: gemini-pro (structured)",
            response: text,
          });
          ...
          return res.json({
            success: true,
            message:
              "Gemini API is working properly with gemini-pro (direct prompt)",
            response: text,
          });
        }
      } catch (fallbackError) {
        throw new Error(
          `All Gemini models failed. Last error: ${fallbackError.message}`
        );
      }
    }
  } catch (error) {
...
// Route to get all doctors associated with a specific location
router.get("/:id/doctors", getLocationDoctors);
