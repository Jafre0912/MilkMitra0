...
const createVeterinarian = async (req, res) => {
  try {
    console.log("Creating veterinarian with data:", req.body);

    // Create the new veterinarian
    const veterinarian = new Veterinarian(req.body);
    const newVeterinarian = await veterinarian.save();

    console.log("Veterinarian created:", newVeterinarian);
...
const updateVeterinarian = async (req, res) => {
  try {
    console.log("Updating veterinarian with ID:", req.params.id);
    console.log("Update data:", req.body);

    // Find and update the veterinarian
    ...
    console.log("Veterinarian updated:", veterinarian);
    res.json(veterinarian);
  } catch (error) {
...
// Get all doctors for a specific location
const getLocationDoctors = async (req, res) => {
  try {
    const location = await Veterinarian.findById(req.params.id);

    if (!location) {
      return res.status(404).json({ message: "Veterinary location not found" });
    }

    if (location.type !== "location") {
      return res
        .status(400)
        .json({ message: "The specified ID is not a location" });
    }

    // Use the method we created to get all doctors
    const doctors = await location.getDoctors();

    res.json(doctors);
  } catch (error) {
    console.error("Error getting location doctors:", error);
    res.status(500).json({ message: error.message });
  }
};
...
const addReview = async (req, res) => {
  try {
    ...
    const totalRating = veterinarian.reviews.reduce(
