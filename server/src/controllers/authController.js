...
exports.getMe = async (req, res) => {
  try {
    // Remove select("-password") to return all fields except password
    const user = await User.findById(req.user.id).select("-password -__v -passwordResetToken -passwordResetExpires");
    if (!user) {
      return res.status(404).json({ error: "User not found" });
    }
