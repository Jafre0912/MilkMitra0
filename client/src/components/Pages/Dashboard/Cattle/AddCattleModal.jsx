...
const AddCattleModal = ({ onClose, onAdd, healthOptions, breedOptions }) => {
  ...
  // Auto-generate tag ID when component loads
  useEffect(() => {
    // Generate a random tag ID with format TAG-XXXX where X is a random digit
    const randomDigits = Math.floor(1000 + Math.random() * 9000); // Random 4-digit number
    const tagId = `TAG-${randomDigits}`;
  ...
  const handleSubmit = async (e) => {
    ...
    if (includePurchaseInfo) {
      if (
        ...
        isNaN(Number(formData.purchaseInfo.cost))
      ) {
  ...
  const isStep1Valid = () => {
    return formData.tagId && formData.name && formData.dateOfBirth;
  };
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Auto-generated tag ID"
                    readOnly
                  />
                  {getFieldError("tagId") && (
                ...
                {/* Removed gender field - using female as default */}
              </div>
            </div>
          )}
            ...
            <div className="space-y-6">
              ...
              <div className="bg-green-50 p-4 rounded-lg border border-green-100 mt-8 flex">
                ...
                <div className="text-sm text-green-700">
                  <p>
                    You're about to register a new female {formData.breed}{" "}
                    cattle named <strong>{formData.name}</strong> with tag ID{" "}
