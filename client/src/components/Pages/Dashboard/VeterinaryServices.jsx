...
import { MapPin, Phone, Clock, Award, Search, User } from "lucide-react";
...
const fallbackResponses = {
  symptom:
    "For livestock showing symptoms like lethargy, reduced appetite, or abnormal behavior, monitor them closely and consult a veterinarian if symptoms persist for more than 24 hours. Keep the animal comfortable, ensure it has access to clean water, and consider isolating it from the herd if an infectious disease is suspected.",
  disease:
    "Common livestock diseases include mastitis, foot rot, respiratory infections, and parasitic diseases. Early detection is key to treatment. Look for changes in behavior, appetite, milk production, and physical symptoms. Maintain good hygiene, proper nutrition, and regular vaccinations to prevent disease spread.",
  treatment:
    "While waiting for veterinary assistance, ensure the animal is comfortable, has access to clean water and appropriate food. For minor injuries, cleaning with diluted antiseptic solution may help. However, avoid administering medications without professional guidance as improper treatment can worsen the condition.",
  prevention:
    "Disease prevention in livestock includes regular vaccinations, proper nutrition, clean housing conditions, biosecurity measures, parasite control, and stress management. Regular health checks and maintaining quarantine protocols for new animals are also essential practices.",
  nutrition:
    "Livestock nutrition requirements vary by species, age, and production stage. A balanced diet should include appropriate amounts of protein, energy, minerals, vitamins, and clean water. Consult a nutritionist to develop a feeding program tailored to your animals' specific needs.",
  emergency:
    "In veterinary emergencies, contact your veterinarian immediately. While waiting, keep the animal calm and comfortable, stop any bleeding with pressure, and don't give food or water if the animal appears to have digestive issues or is struggling to breathe.",
};
...
const VeterinaryServices = () => {
  ...
  const fetchVeterinarians = async () => {
    try {
      ...
      // Ensure we're setting an array of location-type records
      const allData = Array.isArray(response.data) ? response.data : [];

      // Filter to only include locations (exclude doctors)
  ...
  // Get doctors associated with a location
  const getLocationDoctors = (location) => {
    ...
    return doctors.filter((doctor) => location.doctorIds.includes(doctor._id));
  };
    ...
    <div className="container mx-auto px-4 py-6">
      <div className="mb-6">
        ...
        <p className="text-gray-600 mb-4">
          Find trusted veterinary locations with specialized doctors for
          livestock and dairy cattle care near you.
        </p>
        ...
        <div className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search for veterinary locations..."
              ...
              >
                <Popup>
                  <div className="p-1">
                    ...
                    {getLocationDoctors(vet).length > 0 && (
                      <div className="mt-1">
                        <p className="text-sm font-semibold">Doctors:</p>
                        <ul className="text-xs pl-2">
                          {getLocationDoctors(vet).map((doctor) => (
                            <li key={doctor._id}>👨‍⚕️ {doctor.name}</li>
                          ))}
                        </ul>
                      </div>
                    )}
      ...
      {loading ? (
        ...
        <>
          <h2 className="text-2xl font-bold text-gray-800 mb-4">
            {userLocation
              ? "Veterinary Locations Near You"
              : "All Veterinary Locations"}
          </h2>

          {sortedVeterinarians.length === 0 ? (
            <div className="bg-white rounded-xl shadow-lg p-8 text-center">
              <p className="text-gray-600">
                No veterinary locations found matching your criteria.
              </p>
              <button
                onClick={() => {
                  setSearchTerm("");
