...
const Finance = () => {
  ...
  const exportToExcel = () => {
    try {
      ...
      const totalQuantity = milkCollections.reduce(
        (sum, item) => sum + item.quantity,
        0
      );
      const totalAmount = milkCollections.reduce(
        (sum, item) => sum + item.totalAmount,
        0
      );
      const avgFat =
        milkCollections.reduce((sum, item) => sum + item.fat, 0) /
        (milkCollections.length || 1);
      const avgSNF =
        milkCollections.reduce((sum, item) => sum + item.snf, 0) /
        (milkCollections.length || 1);
      const avgRate =
        milkCollections.reduce((sum, item) => sum + item.rate, 0) /
        (milkCollections.length || 1);
      ...
      const startDateStr = collectionFilters.dateRange.startDate
        .toLocaleDateString()
        .replace(/\//g, "-");
      const endDateStr = collectionFilters.dateRange.endDate
        .toLocaleDateString()
        .replace(/\//g, "-");
      const shiftStr = collectionFilters.shift
        ? `_${collectionFilters.shift}`
        : "";
        ...
        >
          <div className="flex justify-between items-center">
            <div>
              ...
              <h3 className="text-xl font-semibold">
                ₹{mppFinanceData.totalAmount.toLocaleString()}
              </h3>
              <p className="text-xs text-gray-400">
                {mppFinanceData.totalQuantity.toFixed(2)} L from{" "}
                {mppFinanceData.count} collections
              </p>
            </div>
