...
const MilkProduction = ({ embedded = false }) => {
  ...
  // Add debug logging for procurement
  useEffect(() => {
    if (sendToProcurement) {
      console.log("Procurement Data:", procurementData);
      console.log("Available Fat/SNF Values:", availableFatSnf);
    }
  }, [sendToProcurement, procurementData, availableFatSnf]);
  ...
  useEffect(() => {
    ...
    const loadFatSnfValues = async () => {
      try {
        ...
        setAvailableFatSnf(values || { fat: [], snf: [] });
        ...
        if (
          ...
          // Log successful loading of values
          console.log("Successfully loaded fat/snf values:", {
            fat: values.fat[0],
            snf: values.snf[0],
          });
        } else {
          console.warn("No fat/snf values available");
        }
      } catch (error) {
        console.error("Failed to fetch FAT/SNF values:", error);
        // Set default empty values on error
        setAvailableFatSnf({ fat: [], snf: [] });
      }
    };
  ...
  const resetForm = () => {
    ...
    // Get available fat/snf values
    const hasFatSnfValues =
      availableFatSnf?.fat?.length > 0 && availableFatSnf?.snf?.length > 0;

    setProcurementData({
      milkType: "C",
      fat: hasFatSnfValues ? availableFatSnf.fat[0] : "",
      snf: hasFatSnfValues ? availableFatSnf.snf[0] : "",
    });
  ...
  const handleDelete = async (id) => {
    try {
      // First find the milk production record to get its data
      const record = milkProductions.find((r) => r._id === id);

      if (!record) {
        toast.error("Could not find the record to delete");
        return;
      }

      // Check if this record was sent to procurement
      const hasProcurementRecord = sentToProcurement[id];

      // Create a confirmation message that indicates if procurement records will also be deleted
      const confirmMessage = hasProcurementRecord
        ? "This will delete both the milk production record AND any associated procurement records. Are you sure you want to continue?"
        : "Are you sure you want to delete this milk production record?";

      // Ask for confirmation
      if (window.confirm(confirmMessage)) {
        // Delete the milk production record
        const result = await deleteMilkProduction(id);

        if (result.success) {
          toast.success("Milk production record deleted successfully!");

          // Calculate the total milk quantity from the record
          const totalMilk =
            (record.morningAmount || 0) + (record.eveningAmount || 0);
          ...
          if (hasProcurementRecord) {
            try {
              // Find matching procurement records by date and quantity
              const procurementRecords = await fetchProcurementRecords();
              const prodDate = new Date(record.date)
                .toISOString()
                .split("T")[0];

              // Find procurement records with matching date and similar quantity
              const matchingRecords = procurementRecords.filter((pr) => {
                const prDate = pr.collectionDate.split("T")[0];
                return (
                  prDate === prodDate && Math.abs(pr.quantity - totalMilk) < 0.5
                );
              });

              // Delete all matching procurement records
              if (matchingRecords.length > 0) {
                console.log(
                  "Found matching MPP records to delete:",
                  matchingRecords
                );

                // Delete each matching procurement record
                const deletePromises = matchingRecords.map((pr) =>
                  deleteMilkCollection(pr._id)
                );
                await Promise.all(deletePromises);

                toast.success(
                  `Deleted ${matchingRecords.length} associated procurement record(s)`
                );

                // Update sent to procurement status
              ...
              console.error(
                "Error deleting associated procurement records:",
                error
              );
              toast.error("Failed to delete associated procurement records");
            }
          }
      ...
      if (result.success) {
        if (sendToProcurement) {
          try {
            ...
            // Validate fat and snf values
            if (!procurementData.fat || !procurementData.snf) {
              toast.error(
                "Please select valid Fat and SNF values for procurement"
              );
              return;
            }
            ...
            if (
              isNaN(mppData.fat) ||
              isNaN(mppData.snf) ||
              isNaN(mppData.quantity)
            ) {
              toast.error("Invalid numeric values in procurement data");
              console.error("Invalid procurement data:", mppData);
              return;
            }
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <div className="flex space-x-2">
                      ...
                      <button
                        onClick={() => {
                          // Check if values are available before setting procurement data
                          const hasFatSnfValues =
                            availableFatSnf?.fat?.length > 0 &&
                            availableFatSnf?.snf?.length > 0;
                          ...
                          // Show warning if no fat/snf values available
                          if (!hasFatSnfValues) {
                            toast.warning(
                              "Fat/SNF values not available. Please select them manually."
                            );
                          }
                        }}
