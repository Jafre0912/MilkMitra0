...
const useDiseaseStore = create((set) => ({
  ...
  predictDisease: async (diseaseData) => {
    ...
    try {
      ...
      if (predictionData.cattleData) {
        ...
        predictionData.cattleId =
          predictionData.cattleData._id || originalCattleId;

        // If cattle has age/breed/gender info and prediction doesn't, use it
        if (
          predictionData.cattleData.age &&
          (!predictionData.age || predictionData.useDefaultValues)
        ) {
          predictionData.age = predictionData.cattleData.age;
        }

        if (
          predictionData.cattleData.breed &&
          (!predictionData.breed || predictionData.useDefaultValues)
        ) {
          predictionData.breed = predictionData.cattleData.breed;
        }

        if (
          predictionData.cattleData.gender &&
          (!predictionData.gender || predictionData.useDefaultValues)
        ) {
  ...
  savePrediction: async (predictionData) => {
    ...
    try {
      ...
      if (
        predictionData.symptoms === undefined ||
        predictionData.symptoms === null
      ) {
      ...
      if (Array.isArray(predictionData.symptoms)) {
        ...
        throw new Error(
          `Invalid symptoms format: ${typeof predictionData.symptoms}`
        );
      }
  ...
  fetchPredictionById: async (id) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // Ensure the prediction has a result object with required properties
        if (!predictionData.result) {
          ...
          // Make sure all required properties exist on the result object
          if (!predictionData.result.possibleDiseases) {
            predictionData.result.possibleDiseases = [];
          }
          if (!predictionData.result.urgencyLevel) {
            predictionData.result.urgencyLevel = "low";
          }
          // Other properties are optional and checked in the UI
        }
